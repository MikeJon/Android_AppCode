...
import com.freerdp.freerdpcore.domain.JzManualBoolmark;
...
import com.jz.vdistack.application.GlobalApp;
import com.jz.vdistack.application.GlobalSettings;
import com.jz.vdistack.application.SessionState;
...
public class SessionActivity extends BaseActivity
	implements LibFreeRDP.UIEventListener, KeyboardView.OnKeyboardActionListener, ScrollView2D.ScrollView2DListener, 
			   ...
			   ClipboardManagerProxy.OnClipboardChangedListener,Runnable
{	
	...
	{		
		...
		private void OnConnectionSuccess(final Context context){
			bindSession();
			rladvertising.setVisibility(View.GONE);
			//JzVDIHomeActivity.jzVDIHomeActivity.finish();
		...
		private void OnDisconnected(final Context context){
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					l.add(new BasicNameValuePair("mac", WcNetWorkUtils.getMacInfo(context)));
					l.add(new BasicNameValuePair("type", "0"));
					String s =WcNetWorkUtils.getResultPost("http://180.153.152.42:3000/connection", l);	
				}
			}).start();
			  is = true;
			}
	...
	private AdvertisingView rladvertising;
	private Thread thread;
	
	private static final String URLHTTP="180.153.152.42:3000/";
	...
	private void processIntent(Intent intent,String str){
		JzManualBoolmark jzManualBoolmark;
		try {
			jzManualBoolmark = new JzManualBoolmark(str,this);
			connect(jzManualBoolmark.getManualBookmark() );
            new Thread(new Runnable() {
				...
				public void run() {
					...
					ll.add(new BasicNameValuePair("mac", WcNetWorkUtils.getMacInfo(SessionActivity.this)));
					ll.add(new BasicNameValuePair("type", "1"));
					String s =WcNetWorkUtils.getResultPost("http://180.153.152.42:3000/connection", ll);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		if(rladvertising.getVisibility()==View.VISIBLE){
			return false;
		}
	...
	public void processVirtualKey(int virtualKeyCode, boolean down) {
		Log.v("WC","processVirtualKey"+virtualKeyCode+down);
