...
public class KeyboardMapper
{
    ...
    private KeyEventInfo keyEventInfo ;    //
	...
	{   
		...
		keymapAndroid[KeyEvent.KEYCODE_DPAD_CENTER] = VK_RETURN | VK_EXT_KEY;
...
//		keymapAndroid[KeyEvent.KEYCODE_ALT_LEFT] =  (KEY_FLAG_UNICODE | 40);
//		keymapAndroid[KeyEvent.KEYCODE_ALT_RIGHT] = (KEY_FLAG_UNICODE | 41);
...
*/
	...
	public void reset(KeyProcessingListener listener) {
		...
		keyEventInfo = KeyEventInfo.getInstance(listener);
	}
	...
	public boolean processAndroidKeyEvent(KeyEvent event) {
		...
		switch(event.getAction())
		{
			...
			{
				keyEventInfo.setIsshift(vkcode,false);
				return false;
			}			
			...
			{	
				...
				keyEventInfo.setIsshift(vkcode,true);
				Log.v("WC",event.getMetaState()+"processAndroidKeyEvent:"+vkcode);
				...
				{   Log.v("WC",event.getMetaState()+"processAndroidKeyEvent:"+KeyEvent.META_ALT_ON+KeyEvent.META_SHIFT_ON+KeyEvent.META_SYM_ON);
				...
				{   Log.v("WC","processAndroidKeyEvent：UNICODE"+event.getUnicodeChar());
	...
	public void processCustomKeyEvent(int keycode) {
		Log.v("WC","processCustomKeyEvent"+keycode);
		...
		Log.v("WC","processCustomKeyEvent：："+extCode);
		resetModifierKeysAfterInput(false);
	}
	...
	private void processToggleButton(int keycode) {
	    Log.v("WC","processToggleButton"+keycode);
		switch(keycode)
		{
			case VK_LSHIFT:
			{
